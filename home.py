...
<head>
    ...
    <style>
        ...
        .menu-toggle:hover { background: #30363d; color: #79c0ff; }
        .neon-text-style { font-family: 'Courier New', Courier, monospace; font-size: 18px; font-weight: bold; color: #fff; text-shadow: 0 0 5px #58a6ff, 0 0 10px #58a6ff, 0 0 20px #0052cc, 0 0 40px #0052cc; animation: neonPulseAnim 1.5s ease-in-out infinite alternate; letter-spacing: 1px; }
        @keyframes neonPulseAnim { from { text-shadow: 0 0 4px #58a6ff, 0 0 8px #58a6ff, 0 0 15px #0052cc, 0 0 30px #0052cc; opacity: 0.9; } to { text-shadow: 0 0 6px #58a6ff, 0 0 14px #58a6ff, 0 0 25px #0052cc, 0 0 50px #0052cc; opacity: 1; } }
        .sidebar-curtain { position: fixed; top: 0; right: -300px; width: 280px; height: 100%; background-color: #161b22; border-left: 2px solid #58a6ff; box-shadow: -10px 0 30px rgba(0,0,0,0.7); z-index: 1000; transition: right 0.3s cubic-bezier(0.4, 0, 0.2, 1); padding: 20px; box-sizing: border-box; text-align: right; }
        .sidebar-curtain.active { right: 0; }
        .close-btn { background: none; border: none; color: #f85149; font-size: 16px; cursor: pointer; margin-bottom: 30px; display: flex; align-items: center; gap: 8px; font-family: inherit; font-weight: bold; }
        .menu-links { display: flex; flex-direction: column; gap: 12px; }
        .menu-item { display: flex; align-items: center; gap: 12px; color: #c9d1d9; text-decoration: none; font-weight: bold; font-size: 15px; padding: 12px; border: 1px solid #30363d; border-radius: 6px; background: #21262d; transition: all 0.2s ease; }
        .menu-item:hover { border-color: #58a6ff; color: #58a6ff; background: rgba(88, 166, 255, 0.05); transform: translateX(-5px); }
        .main-container { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 30px 20px; }
        .dev-portfolio-card { background: #161b22; border: 1px solid #30363d; border-top: 4px solid #58a6ff; border-radius: 12px; padding: 35px 25px; max-width: 550px; width: 100%; box-shadow: 0 20px 40px rgba(0,0,0,0.6); box-sizing: border-box; text-align: right; position: relative; }
        .terminal-header { display: flex; gap: 6px; position: absolute; top: 12px; left: 15px; }
        .dot { width: 10px; height: 10px; border-radius: 50%; }
        .dot-r { background: #f85149; }
        .dot-y { background: #d29922; }
        .dot-g { background: #3fb950; }
        .dev-avatar-img { width: 110px; height: 110px; border-radius: 16px; object-fit: cover; border: 2px solid #58a6ff; display: block; margin: 0 auto 20px auto; box-shadow: 0 0 20px rgba(88, 166, 255, 0.3); background: #0d1117; }
        ...
        .skills-container { display: flex; flex-direction: column; gap: 6px; margin-top: 10px; padding-right: 15px; }
        .skill-badge { color: #3fb950; font-size: 13px; font-weight: bold; display: block; text-align: right; }
    </style>
</head>
<body>
    ...
    <div class="main-container">
        <div class="dev-portfolio-card">
            <div class="terminal-header"><div class="dot dot-r"></div><div class="dot dot-y"></div><div class="dot dot-g"></div></div>
            <!-- 🎯 يقرأ الصورة محلياً من ملف static/avatar.png المرفوع في حاوية GitHub الخاصة بك -->
    ...
    <script>
        function toggleSidebarCurtain(open) { const curtain = document.getElementById('sidebarCurtain'); if (open) { curtain.classList.add('active'); } else { curtain.classList.remove('active'); } }
