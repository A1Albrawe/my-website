...
# عزل التنسيقات المتجاوبة الذكية لعام 2026 لضمان ثبات البناء ومنع تسريب الشفرة خارج الصندوق
HOME_CSS = """
<style>
    ...
    /* 🌐 إصلاح انعكاس شريط التنقل العلوي بالملي: الاسم يميناً والقائمة يساراً */
    ...
    .brand-logo { font-size: 22px; font-weight: bold; color: #fff; text-shadow: 0 0 8px #58a6ff; font-family: monospace; order: 1; }
    ...
    /* 💻 تعريض المنظومة وهندستها لتناسب الكمبيوتر والموبايل معاً بكفاءة تامة */
    .responsive-profile-wrapper { display: flex; flex-direction: row; gap: 25px; width: 100%; max-width: 1000px; background: #0d1117; border: 1px solid #30363d; border-radius: 16px; padding: 30px; box-shadow: 0 25px 50px rgba(0,0,0,0.6); border-bottom: 4px solid #58a6ff; box-sizing: border-box; align-items: stretch; direction: rtl; }
    
    .profile-sidebar-zone { flex: 0 0 220px; display: flex; flex-direction: column; align-items: center; text-align: center; border-left: 1px solid #21262d; padding-left: 20px; box-sizing: border-box; }
    .profile-content-zone { flex: 1; display: flex; flex-direction: column; justify-content: center; text-align: right; box-sizing: border-box; }
    
    /* 📱 شاشة الاستجابة التلقائية لهواتف الموبايل: تتحول لعمودية رشيقة تلقائياً */
    @media (max-width: 768px) {
        .top-nav { max-width: 100%; }
        .responsive-profile-wrapper { flex-direction: column; align-items: center; padding: 20px; max-width: 440px; }
        ...
        .profile-content-zone { width: 100%; }
    }
    .avatar-wrapper { width: 130px; height: 130px; border-radius: 14px; border: 2px solid #58a6ff; overflow: hidden; box-shadow: 0 0 15px rgba(88,166,255,0.25); margin-bottom: 15px; display: flex; align-items: center; justify-content: center; background: #04060a; }
    .avatar-img { width: 100%; height: 100%; object-fit: cover; }
    
    .profile-name { font-size: 24px; font-weight: bold; color: #fff; margin: 0 0 6px 0; text-shadow: 0 0 5px rgba(255,255,255,0.2); letter-spacing: 0.5px; }
    ...
    .details-sub-box { display: flex; flex-direction: column; gap: 14px; font-size: 13.5px; line-height: 1.6; }
    ...
    /* 🕹️ الـ Sidebar الجانبي المنزلق الأصلي النيوني للـ الأركيد */
    .sidebar-overlay { position: fixed; top: 0; right: -100%; width: 100%; max-width: 290px; height: 100vh; background: rgba(13, 17, 23, 0.98); border-left: 2px solid #58a6ff; box-shadow: -10px 0 30px rgba(0, 0, 0, 0.7); z-index: 9999; display: flex; flex-direction: column; padding: 20px; box-sizing: border-box; transition: right 0.35s cubic-bezier(0.4, 0, 0.2, 1); overflow-y: auto; }
...
def home_page():
    # 🧠 تفعيل محرك التوليد التلقائي لـ الألعاب من مجلد static/my_games/
    dynamic_games_html = ""
    try:
        ...
        dynamic_games_html = '<p style="color:#8b949e; font-size:12px;">خطأ في تحميل باقة ألعاب النظام التلقائية.</p>'

    if not dynamic_games_html:
        dynamic_games_html = '<p style="color:#8b949e; font-size:12px;">قائمة ألعاب النظام فارغة حالياً.</p>'
...
<body>
    ...
    <div class="main-container">
        <!-- 🛡️ الهيكل ثنائي الأجنحة الفاخر المتجاوب مع الشاشات العملاقة والموبايل حياً -->
        <div class="responsive-profile-wrapper">
            
            <!-- الجناح الأيمن (خاص بالصورة والاسم والمسمى الوظيفي) -->
            <div class="profile-sidebar-zone">
                <div class="avatar-wrapper">
                    <!-- ✅ الملاءمة الدقيقة لمسار الأفاتار الصافي من الجذر ليتعرف عليه Vercel فوراً ويظهره -->
            ...
            <!-- الجناح الأيسر (خاص بالنبذة ومجالات الخبرة والتقنيات القياسية) -->
