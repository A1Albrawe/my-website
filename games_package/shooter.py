...
<head>
    ...
    <style>
        ...
        /* 🕹️ أزرار التحكم العملاقة والمنفصلة للعب الحربي السلس والمستقر للتاتش */
        .control-pad { margin-top: 15px; display: grid; grid-template-columns: 1fr 1.2fr 1fr; gap: 12px; width: 100%; }
        .ctrl-btn { background: #21262d; border: 1px solid #30363d; border-radius: 16px; padding: 16px; font-size: 22px; color: #388bfd; cursor: pointer; user-select: none; -webkit-user-select: none; font-weight: bold; box-shadow: 0 5px #0d1117; transition: 0.1s; display: flex; align-items: center; justify-content: center; touch-action: none; }
        ...
        .pause-action-btn { grid-column: span 3; background: #21262d; border: 1px solid #8b949e; color: #8b949e; font-size: 13px; font-weight: bold; border-radius: 10px; padding: 12px; cursor: pointer; box-shadow: 0 4px #0d1117; display: flex; align-items: center; justify-content: center; gap: 8px; margin-top: 5px; font-family: inherit; user-select: none; -webkit-user-select: none; touch-action: none; }
...
<body>
    ...
    <div class="main-container">
        <div class="shooter-arcade-cabinet">
            ...
            <!-- 🕹️ هندسة دعم أحداث اللمس الفوري والمطوّل للهواتف الذكية مع منع الاهتزاز والزحف -->
            <div class="control-pad">
                <button class="ctrl-btn" 
                        ontouchstart="event.preventDefault(); moveLeft(true)" 
                        ontouchend="event.preventDefault(); moveLeft(false)"
                        onmousedown="moveLeft(true)" 
                        onmouseup="moveLeft(false)"><i class="fas fa-chevron-left"></i></button>
                        
                <button class="ctrl-btn fire-btn" 
                        ontouchstart="event.preventDefault(); shootLaser()" 
                        onclick="shootLaser()"><i class="fas fa-crosshairs"></i></button>
                        
                <button class="ctrl-btn" 
                        ontouchstart="event.preventDefault(); moveRight(true)" 
                        ontouchend="event.preventDefault(); moveRight(false)"
                        onmousedown="moveRight(true)" 
                        onmouseup="moveRight(false)"><i class="fas fa-chevron-right"></i></button>
                
                <button class="pause-action-btn" 
                        ontouchstart="event.preventDefault(); togglePause()" 
                        onclick="togglePause()"><i class="fas fa-pause"></i> تعليق القتال / استئناف الهجوم</button>
            </div>
        </div>
    </div>
    <script>
        ...
        function drawPlayerShip(x, y, w, h) {
            ...
            ctx.moveTo(x + w / 2, y); ctx.lineTo(x, y + h); ctx.lineTo(x + w, y + h); ctx.closePath(); ctx.fill();
            ctx.strokeStyle = '#58a6ff'; ctx.lineWidth = 2; ctx.strokeRect(x - 2, y + h - 6, w + 4, 3);
        }

        function drawEnemyShip(x, y, w, h) {
            ctx.fillStyle = '#f85149'; ctx.fillRect(x + 4, y, w - 8, h - 4);
            ctx.fillStyle = '#ff7b72'; ctx.fillRect(x, y + h - 4, w, 4);
        }
