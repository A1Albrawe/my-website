...
<head>
    ...
    <style>
        body { font-family: 'Courier New', Courier, monospace; background: #0d1117; color: #c9d1d9; padding: 0; margin: 0; display: flex; flex-direction: column; min-height: 100vh; box-sizing: border-box; overflow-x: hidden; }
...
<body>
    ...
    <script>
        ...
        // ✅ دالة ربط الرادار وحقن وحفظ بيانات تصفح الزائر بالثانية حياً داخل لوحة المسؤول
        document.addEventListener("DOMContentLoaded", () => {
            ...
            setInterval(() => {
                fetch('/api/update_duration', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ username: storedUser, game: 'site' })
                });
            }, 5000);
        });
    </script>
...
def home_page():
    ...
    return render_template_string(HOME_TEMPLATE.replace("SIDEBAR_LINKS_PLACEHOLDER", dynamic_links))
